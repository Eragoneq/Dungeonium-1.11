package net.eragon_skill.handlers;
...
/**
 * ModelHandler and other render handling stuff are used from Botania mod.
 * https://github.com/Vazkii/Botania
 */
public final class ModelHandler {
	
	public static void registerModels(){
		OBJLoader.INSTANCE.addDomain(References.MOD_ID.toLowerCase(Locale.ROOT));
	...
	public static void registerItemMetas(Item item, int maxExclusive, IntFunction<String> metaToName) {
		for (int i = 0; i < maxExclusive; i++) {
			ModelLoader.setCustomModelResourceLocation(
					item, i,
					new ModelResourceLocation(References.MOD_ID + ":" + metaToName.apply(i), "inventory")
					);
		}
	}
	...
	public static void registerCustomItemblock(Block b, int maxExclusive, IntFunction<String> metaToPath) {
		...
		for (int i = 0; i < maxExclusive; i++) {
			ModelLoader.setCustomModelResourceLocation(
					item, i,
					new ModelResourceLocation(References.MOD_ID + ":itemblock/" + metaToPath.apply(i), "inventory")
					);
		}
	}
}
